...
describe Merchant::CreditBacksController do
  ...
  describe 'create action' do
    it 'should create credit back plan' do
      ...
      post :create, merchant_credit_back: payment_plan, merchant_store_id: @merchant_store.id
      response.code.should eq('302')
      flash[:error].should be_nil
      flash[:success].should_not be_nil
      pp = Pay::CreditBack.where(:merchant_store_id => @merchant_store.id).first
      pp.should_not be_nil
      pp.merchant_rate.should be == 0.12
      pp.customer_rate.should be == 0.12
  ...
  describe 'update action' do
    before :each do
      @payment_plan = create(:credit_back, :invalid, merchant_store: @merchant_store)
    end
    it 'should update credit_back payment plan' do
      post_payment_plan = {
          merchant_rate: 12,
          referer_rate: 12,
          valid_from: Time.now.strftime(FormFieldsConversion::DATE_TIME_FORMAT),
      ...
      post :update, :merchant_credit_back => post_payment_plan, :merchant_store_id => @merchant_store.id, :id => @payment_plan.id
      response.code.should eq('302')
      flash[:error].should be_nil
      flash[:success].should_not be_nil
      pp = Pay::CreditBack.find(@payment_plan.id)
      pp.should_not be_nil
